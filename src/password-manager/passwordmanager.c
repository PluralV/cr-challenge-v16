...
int password_retrieve(char *cat){
    FILE *pw_file;
    if ((pw_file = fopen(PASSWORD_FILE_PATH,"r")) == NULL) return -1;
    char current_char;
    int i = 0;
    ...
    fclose(pw_file);
    printf("No password exists for identifier \"%s.\" Try checking your spelling.\n",cat);
    return -1;
}

int main(int argc, char **argv){
    if (argc <= 1 || **(argv+1)!='-'){
        printf("ERROR: %s FIRST ARGUMENT: %s\nSee below for proper usage:\n%s",(argc > 1) ? "INVALID" : "MISSING",*(argv+1),USAGE);
        return -1;
    }
    switch(*(*(argv+1)+1)){
        case 'h':
